    ...
    try {
      ...
      if (data.success) {
        ...
        if (task.type === 'REFERRAL' && data.currentReferrals !== undefined && data.requiredReferrals !== undefined) {
          ...
          showErrorMessage(`You need ${remainingReferrals} more referral${remainingReferrals > 1 ? 's' : ''} to complete this task. (${data.currentReferrals}/${data.requiredReferrals})`);
        } else {
          showErrorMessage(data.message || `Failed to complete ${task.type} task. Please try again.`);
        }
      }
    } catch (error) {
      console.error('Error checking task:', error);
      showErrorMessage(error instanceof Error ? error.message : `Failed to check ${task.type} task. Please try again.`);
    } finally {
      setIsLoading(false);
    }
  };
